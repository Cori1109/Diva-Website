...
from 800 members to over 15k members. We can already announce that there will be another round of incentivized testnet where users will get a chance to earn additional rewards for trying out new features that we are currently
developing for the app. Below are some relevant updates on the on-going test.
...
all parts of the app and testnet to Goerli, the new Ethereum test network. This move was expected and most of the preparations for the migration have been done on our side. Unfortunately, one important piece of infrastructure 
that the DIVA App is using, namely the 0x api server which is maintained by the 0x protocol team and responsible for hosting signed trade orders, turned out to have limited functionality on Goerli. As a result, users
were no longer able to trade and complete the four trade related testnet tasks. We are currently in the process of spinning up the api server ourselves to become and confident to enable the trading functionality soon. 
...
Overall, no critical bugs anymore. All bugs reported during the early phase of the testnet were fixed and users are able to complete all tasks without any problems (assuming the trading part is functional again).
