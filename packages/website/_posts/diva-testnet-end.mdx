...
to filter out multi-accounts. Roughly 1'000 alt accounts and their corresponding main accounts (3.7m or 4.7% of total points) have been filtered out that way.
...
Despite sufficient heads-up, more than 11k registered users (c. 25m or 31.4% of total points) missed the verification process. As users have spent a 
...
It's important to highlight that those rewards come on top of the initially planned reward pool size and do not dilute verified users. Overall, a total of 1.37m DIVA tokens have been allocated to more 
than 25'200 users. That is 37% more than originally planned.
...
**Note:** Until token launch, we reserve the right to blacklist multi-accounts that managed to verify if we are presented credible evidence.
